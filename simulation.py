...
#         # DataFrames separados para almacenar los mismos datos que los diccionarios
#         self.price_history_df = pd.DataFrame({crypto: [market.cryptocurrencies[crypto].price] for crypto in market.cryptocurrencies})
#         self.volume_history_df = pd.DataFrame({crypto: [] for crypto in market.cryptocurrencies})
#         self.sentiment_history_df = pd.DataFrame({crypto: [] for crypto in market.cryptocurrencies})
#         self.agent_performances_df = pd.DataFrame({agent.nombre: [] for agent in agents})
...
#                 self.sentiment_history_df.at[step, crypto] = self.precomputed_sentiments[crypto][step]

#             # Actualiza el estado del mercado con los sentimientos actuales
#             self.market.update(self.sentiment_history)
...
#                 ganancia_actual = agent.historia_ganancia[-1]
#                 self.agent_performances[agent.nombre].append(ganancia_actual)
#                 self.agent_performances_df.at[step, agent.nombre] = ganancia_actual
...
#                     self.agent_performances_df[nuevo_nombre] = np.nan  # Añadir columna en el DataFrame
#                     No_Agente += 1
